...
def test_continuation_tracks_default_branch_to_amplitude_one(default_branch_records):
    amplitudes = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
    records = default_branch_records
...
def test_continued_branch_reaches_below_free_mass_energy_per_charge(default_branch_records):
    records = default_branch_records
...
def test_every_accepted_point_has_small_relative_virial_residual(default_branch_records):
    assert all(
        r.relative_virial_residual < 1e-3
        for r in accepted_records(default_branch_records)
    )
