...
| Toroidal circulation | Coarse spacing/curvature scan and sampled clearance frontier implemented; compact reference still collides while tested wider geometry contains collision-free samples | Refine each sampled collision/free bracket, check for re-entrant behavior, then test dimensionless clearance ratios across Flower/Tree scales and recursive depths. Use separate-axis fan-out only if no compact scale-consistent region survives. |
