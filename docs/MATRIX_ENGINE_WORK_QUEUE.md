...
| Toroidal circulation | Shared routing of the parallel Vesica returns gives a separated endpoint with conservative piece bounds, fixed junction frames, and preserved flux; the direct interpolation still collides | Audit the shared return endpoint against the existing junction/connector embedding contract. A safe motion needs a collision-free starting geometry and a separately checked path. General fan-out, dormant-path semantics, and recursive topology claims remain open. |
