...
| Toroidal circulation | Coarse scan, sampled clearance frontier, and interior bracket refinement implemented; compact reference still collides while tested wider geometry contains collision-free samples | Surface the deterministic refined-bracket report, increase local sampling for any re-entrant region, then test dimensionless clearance ratios across Flower/Tree scales and recursive depths. Use separate-axis fan-out only if no compact scale-consistent region survives. |
