...
| Toroidal circulation | Sampling reliability exposes missed collisions; spherical inversion preserves the known intersections and flux with matched orientations, while straight interpolation to it is singular | Develop conservative clearance bounds and specify dormant-path semantics before Flower/Tree depth claims. For geometry changes, define a relative rearrangement or orientation-preserving deformation and check intermediate collisions, Jacobians, and flux; common inversion alone is not a repair. |
