...
| Toroidal circulation | Separated connector shells verified; the compact reference collides, while a broader spacing/curvature grid contains at least one collision-free point | Map the collision/no-collision boundary, test scale consistency across Flower/Tree and recursive depths, and derive a conservative clearance rule. Use separate-axis fan-out only if no compact scale-consistent region survives. |
